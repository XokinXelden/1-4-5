...
export const AddEditTaskModal = ({ onClose }: onCloser) => {
  const actualTarget = useTypedSelector(
    (state) => state.todoReducer.actualTargetID
  );
  const editOrAdd = actualTarget?.id;
        ...
        <div className={style.addEditModal}>
          <div className="flx-between">
            <span className={style.modalTitle}>
              {editOrAdd ? "Редактировать задачу" : "Добавить задачу"}
            </span>
          ...
          <div className="flx-right mt-50">
            <Button
              title={editOrAdd ? "Редактировать " : "Добавить"}
              onClick={() => {
                editOrAdd
